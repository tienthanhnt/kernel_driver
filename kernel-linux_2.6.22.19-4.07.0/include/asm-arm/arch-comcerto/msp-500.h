/*
 *  linux/include/asm-arm/arch-comcerto/msp-500.h
 *
 ...
 */
#ifndef __ASM_ARCH_COMCERTO_MSP_500_H
#define __ASM_ARCH_COMCERTO_MSP_500_H


#define SMI_CONFIG_VADDR		((void *)(IRAM_MEMORY_VADDR))
#define CBL_CONFIG_VADDR		((void *)(IRAM_MEMORY_VADDR + 0x1250))
...
#if defined(ARAM_MEMORY_SIZE)
/* M825xx1 has no ARAM, exclude these functions */
...
static inline void *msp_iram_to_virt(ulong phys_addr)
{
	return (void*)(phys_addr - IRAM_MEMORY_PHY + IRAM_MEMORY_VADDR);
}

static inline ulong msp_virt_to_iram(void *virt_addr)
{
	return (ulong)virt_addr - IRAM_MEMORY_VADDR + IRAM_MEMORY_PHY;
}
...
static inline void *msp_phys_to_virt(ulong phys_addr)
{
	...
	if (CHECK_ADDR(phys_addr, IRAM_MEMORY_PHY, IRAM_MEMORY_SIZE))
		return msp_iram_to_virt(phys_addr);
#if defined(ARAM_MEMORY_SIZE) && (ARAM_MEMORY_SIZE > 0)
	if (CHECK_ADDR(phys_addr, ARAM_MEMORY_PHY, ARAM_MEMORY_PHY + ARAM_MEMORY_SIZE))
...
static inline ulong msp_virt_to_phys(void *virt_addr)
{
	...
	if (CHECK_ADDR(virt_addr, IRAM_MEMORY_VADDR, IRAM_MEMORY_SIZE))
		return msp_virt_to_iram(virt_addr);
#if defined(ARAM_MEMORY_SIZE) && (ARAM_MEMORY_SIZE > 0)
...
#endif /* __ASM_ARCH_COMCERTO_MSP_500_H */
